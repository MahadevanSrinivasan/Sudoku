...
for iter = 1 : maxIter
    fprintf('Iteration: %d\n',iter);
    [rowind colind] = find(Solution == 0);
    vectorIndex = find(Solution == 0);
    if(~length(vectorIndex))
        disp('Problem Solved, exiting...');
        problemSolved = 1;
        break;
    end
    [rowind colind];
    ...
    for i = 1 : length(rowind)
        ...
        possibleRowValues = setdiff(allowedValues, Solution(currRowInd,:));
        possibleColValues = setdiff(allowedValues, Solution(:,currColInd));
        possibleCellValues = setdiff(allowedValues, Solution(cellDefinitions(cellInd,:)));
        ...
        if(length(possibleValues) == 1)
            Solution(vectorIndex(i)) = possibleValues;
        end
    end
